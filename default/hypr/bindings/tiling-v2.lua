-- Current tiling keymap (the pre-v2 one lives in tiling.lua and is not sourced).
...
-- `drag` is the 0.56 replacement for the old bindm; the `mouse = true` used by
-- Hyprland's own example config is not a real option and is silently ignored.
